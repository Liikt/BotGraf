...
async def changesettings(client, message, admirals, secr):
    ...
    if int(msg.content) == 1:
        ...
        msg = await clientclient.wait_for_message(channel=message.channel,author=message.author)
        ...
        if message.server is not None:
            ...
            while answ == "n":
                ...
                embeded = discord.Embed(color=discord.Colour(0xe67e22))
                embeded.set_image(url=link.content)

                try:
                    await client.send_message(user, embed=embeded)
                    await client.send_message(user, "Is this okay? Y/n")
